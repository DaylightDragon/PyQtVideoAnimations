...
from WindowsAnim.Canvas import Canvas
...
class MainWindow(QMainWindow):
    def __init__(self, data: Data):
        ...
        self.setGeometry(0, 0, data.renderWindowSize[0], data.renderWindowSize[1])
    ...
    def keyReleaseEvent(self, event: QKeyEvent):
        if Qt.Key.Key_Return == event.key():
            data.cameraMovement.smoothMoveTo(targetX=0, targetY=0, targetScale=5, duration=5, realDurationCoef=0.2, easingFunction=Easings.easeInOutQuad)
...
if __name__ == "__main__":
    StylesManager.init()
    data = Data(fps=144, size=(1920, 1080))
